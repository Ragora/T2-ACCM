...
function DefaultGame::spawnPlayer( %game, %client, %respawn ) {
   if($UseForcedTeamSpawn[%client.team])
      if(%client.SP != $ForcedSpawn[%client.team])
         %client.SP = $ForcedSpawn[%client.team];

   if($Rank::Squad[%client.ranknum] !$= "" && %client.spawnOnLead == 1 && $Host::Purebuild == 0){
      %count = ClientGroup.getCount();
      for(%i = 0; %i < %count; %i++){
	   %cl = ClientGroup.getObject(%i);
	   if(%cl.namebase $= $squad::Leader[getWord($Rank::Squad[%client.ranknum],1)]){
	      %leader = %cl;
	      %i = %count;
	   }
      }
      if(%leader.team == %client.team && isObject(%leader.player)){
	   if(%leader.player.mountedtoV){
	      %veh = %leader.player.Vmountedto;
	      for(%i = 0; %i < %veh.getdatablock().numMountPoints; %i++){
		   if(%veh.getMountNodeObject(%i) == 0){
		      %client.lastSpawnPoint = whereToLook("0 0 0 0 0 1 0");
		      %mountToVeh = %i;
		      %i = %veh.getdatablock().numMountPoints;
		   }
		   else if(%i >= (%veh.getdatablock().numMountPoints - 1))
	   		%client.lastSpawnPoint = %game.pickPlayerSpawn( %client, false );
	      }
	   }
	   else
	     %client.lastSpawnPoint = RankFindSpawnPoint(%leader);
      }
      else if(isObject(%client.SP) && %client.SP.team == %client.team && %client.SP.active == 1){
	   %pos = vectorAdd(%client.SP.getPosition(), %client.SP.getUpVector());
	   %pos = vectorAdd(%pos,"0 0 -0.7");
	   %client.lastSpawnPoint = whereToLook(%pos SPC "0 0 1 0");
	}
	else
	   %client.lastSpawnPoint = %game.pickPlayerSpawn( %client, false );
   }
   ...
   if(%mountToVeh !$= ""){
    ...
    if (($Host::Purebuild == 0) && (Game.class $= "InfectionGame") || (Game.class $= "CombatConGame")) {
       MessageClient(%client, "Msg", "~wfx/misc/heartbeat.wav");
...
function RankFindSpawnPoint(%leader){
   %pos = %leader.player.getPosition();
   for(%i = 0; %i < 3; %i++){
      %x = getWord(%pos, 0) + getRandom(3,10);
      %y = getWord(%pos, 1) + getRandom(3,10);
      %z = getWord(%pos, 2) + 3;
      %startpos = %x@" "@%y@" "@%z;
      ...
      %searchResult = containerRayCast(%startpos, vectorAdd(%startpos, "0 0 -6"), %mask, %leader.player);
	if(%searchResult){
	   %trns = getWord(%searchResult, 1)@" "@getWord(%searchResult, 2)@" "@getWord(%searchResult, 3)@ " 0 0 1 0";
	   %loc = whereToLook(%trns);
	   return %loc;
	}
   }
   %x = getWord(%pos, 0) + getRandom(3,10);
...
// jff: this should probably be checking the team of the client
   ...
   if((%client.team == $ZombieTeam) && ($ZombieTeam != 0 && $ZombieTeam !$= ""))
      MakePersonZombie(%player.getTransform(), %client, getRandomB());
}
...
{
   // Sentinels don't need a player object.
   if(%client.nameBase $= "_AISent")
      return;
...
function DefaultGame::startMatch(%game)
{
   ...
   if(!isObject(%zgroup)) {
      %zgroup = new SimGroup("ZombieGroup");
      MissionCleanup.add(%zgroup);
   }
   
   %sgroup = nameToID("MissionCleanup/SentinelGroup");
   if(!isObject(%sgroup)) {
      %sgroup = new SimGroup("SentinelGroup");
      MissionCleanup.add(%sgroup);
   }
   
   %mgroup = nameToID("MissionCleanup/MonitorGroup");
   if(!isObject(%mgroup)) {
      %mgroup = new SimGroup("MonitorGroup");
      MissionCleanup.add(%mgroup);
   }
   ...
   if(isObject(ZombieTriggers))
   {
      for(%d = 0; %d < ZombieTriggers.getCount(); %d++)
      {
         %dude = ZombieTriggers.getObject(%d);
         if(!isEventPending(%dude.zombieLoop))
            ZombieSpawnTrigger::SpawnLoop(ZombieSpawnTrigger, %dude);
      }
   }
...
function DefaultGame::clearDeployableMaxes(%game)
{
   for(%i = 0; %i <= %game.numTeams; %i++)
   {
      ...
      $TeamDeployedCount[%i, waypointDeployable] = 0;
	$TeamDeployedCount[%i, SpawnPointPack] = 0;
      ...
      $TeamDeployedCount[%i, TurretSentryPack] = 0;
   }
}
...
//[[CHANGE]] Make sure the beacon get's removed.. as it should be.. :D
   ...
   if(isObject(%clVictim.isSwitched))
   {
      Game.forceObserver(%clVictim.isSwitched, "AdminForce");
      // Message was removed since it wasn't working...
      %clVictim.isSwitched.isSwitched = "";
      %clVictim.isSwitched = "";
   }
		...
		if ($Host::Prison::KillTime > 0) {
			if ($Host::Prison::KillTime >= 60) {
				if ($Host::Prison::KillTime > 60) {
					...
					messageClient(%clKiller,'msgClient','\c2You will do\c3 %2 \c2minutes in jail for killing\c3 %1\c2.',%victimName,%minutes);
					messageAllExcept(%clKiller,-1,'msgClient','\c3%1 \c2will do\c3 %3 \c2minutes in jail for killing\c3 %2\c2.',%clKiller.name,%victimName,%minutes);
				}
				else {
					messageClient(%clKiller,'msgClient','\c2You will do 1 minute in jail for killing\c3 %1\c2.',%victimName);
					messageAllExcept(%clKiller,-1,'msgClient','\c3%1 \c2will do 1 minute in jail for killing\c3 %2\c2.',%clKiller.name,%victimName);
				}
			}
			else {
				messageClient(%clKiller,'msgClient','\c2You will do\c3 %2 \c2seconds in jail for killing\c3 %1\c2.',%victimName,$Host::Prison::KillTime);
				messageAllExcept(%clKiller,-1,'msgClient','\c3%1 \c2will do\c3 %3 \c2seconds in jail for killing\c3 %2\c2.',%clKiller.name,%victimName,$Host::Prison::KillTime);
			}
		}
		else {
			messageClient(%clKiller,'msgClient','\c2You were put in jail for killing\c3 %1\c2.',%victimName);
			messageAllExcept(%clKiller,-1,'msgClient','\c3%1 \c2was put in jail for killing\c3 %2\c2.',%clKiller.name,%victimName);
		}
   ...
   if (%damageType == $DamageType::Suicide)
   {
	...
	ZKillUpdateScore(%game,%clkiller,%implement);
}
...
//   }
   ...
   if( !%adminForce )
      messageAllExcept(%client, -1, 'MsgClientJoinTeam', '\c3%1 \c2has become an observer.', %client.name, %game.getTeamName(0), %client, 0 );
   else
      messageAllExcept(%client, -1, 'MsgClientJoinTeam', '\c2The admin has forced \c3%1 \c2to become an observer.', %client.name, %game.getTeamName(0), %client, 0 );
...
function DefaultGame::clientMissionDropReady(%game, %client)
{
   ...
   schedule(0, 0, "ccMenu", %client, "Join");
   messageClient(%client, "", "~wfx/Bonuses/Nouns/Colonel.wav");
}
...
{
  if (%clVictim.isSentinel && !%clAttacker.isAIControlled())
  {
    %clVictim.player.UnderFire = true;

//      if ($SN::attacker[%clAttacker] $= "")
//        $SN::attacker[%clAttacker] = 1;
//      else
//        $SN::attacker[%clAttacker] = $SN::attacker[%clAttacker]++;

//        if ($SN::attacker[%clAttacker] > 5)
//          $SN::CleartoAttack(%clAttacker);

      MessageClient(%clAttacker, 'SN_Msg', "\c2Sentinel Network ::\c0 Do not fire upon our units.");
  }
}
...
{
  if (%clVictim.isZombie)
  {
    if (%clVictim.cp !$= "")
      %clVictim.respawnThread = schedule(8000, 0, "ZombieSpawn", %clVictim.cp, %clVictim, %clVictim.SpawnPoint);

     %clVictim.drop(); // Eolk - use drop for bots.
     return;
  }
  if (%clVictim.isSentinel)
  {
// Eolk - moved to elsewhere.
//    %p = new TracerProjectile()
//    {
//       DataBlock        = "SentinelDeathProj";
...
//     MissionCleanup.Add(%p);
     ...
     // Eolk - the player object doesn't exist anymore.
     %clVictim.respawnThread = schedule(10000, 0, "SentinelSpawn", %clVictim, %clVictim.SpawnPoint);
     return;
  }
...
function DefaultGame::sendGamePlayerPopupMenu( %game, %client, %targetClient, %key )
{
   ...
   if ( %game.scheduleVote $= "" && !%isAdmin && !%isTargetAdmin )
   {
	 ...
	 messageClient( %client, 'MsgPlayerPopupItem', "", %key, "AdminPlayer", "", 'Vote to Make Admin', 2 );
      if ( $Host::AllowKeeperPlayerVotes && !%isTargetBot )
         if(!%targetClient.isZombieKeeper)
            messageClient( %client, 'MsgPlayerPopupItem', "", %key, "MakeKeeper", "", 'Vote to Make Keeper', 14 );
         else
            messageClient( %client, 'MsgPlayerPopupItem', "", %key, "MakeNotKeeper", "", 'Vote to Remove Keeper', 14 );
   ...
   {
      if ( %client.isSuperAdmin && !%isTargetBot && !%isTargetAdmin )
	 messageClient( %client, 'MsgPlayerPopupItem', "", %key, "AdminPlayer", "", 'Make Admin', 2 );

      if ( !%isTargetBot && !%isTargetAdmin )
         if(!%targetClient.isZombieKeeper)
            messageClient( %client, 'MsgPlayerPopupItem', "", %key, "MakeKeeper", "", 'Make Keeper', 14 );
         else
            messageClient( %client, 'MsgPlayerPopupItem', "", %key, "MakeKeeper", "", 'Make Not Keeper', 14 );
	 ...
	 if ( !%isTargetBot )
	 {
	    ...
	    if ( !%isTargetObserver && Game.class !$= "ZombieGame" )
	       messageClient( %client, 'MsgPlayerPopupItem', "", %key, "ToObserver", "", 'Force observer', 5 );
	 }


      if ( ( %isTargetSelf || %outrankTarget ) && Game.class !$= "ZombieGame" )
      {
	 if ( %game.numTeams > 1 )
	 {
	    if ( %isTargetObserver )
...
function DefaultGame::sendGameVoteMenu( %game, %client, %key )
{
   ...
   if(%client.BuildingManagerMode == 1)
   {
      messageClient( %client, 'MsgVoteItem', "", %key, 'SavePieces', "", 'Save a Building' );
      messageClient( %client, 'MsgVoteItem', "", %key, 'LoadPieces', "", 'Load a Building' );
      messageClient( %client, 'MsgVoteItem', "", %key, 'RenamePieces', "", 'Rename a Building' );
      messageClient( %client, 'MsgVoteItem', "", %key, 'DeletePieces', "", 'Delete a Building' );
      messageClient( %client, 'MsgVoteItem', "", %key, 'GoBackPiece', "", '[\c1Go Back\c0]' );
      return;
   }

   if(%client.BuildingManagerMode == 2)
   {
      %path = "Buildings/ClientSaves/"@%client.guid@"/*";
      %count = 0;
      for(%f = findFirstFile(%path); %count < $Host::MaxClientSaves; %f = FindNextFile(%path))
      {
         if(fileExt(%f) $= ".dso")
            continue; // DSOs should not take spots.

         if(%f $= "")
            %name = "(empty)";
         else
         {
            %duh = getsubstr(%f, 22, 255);
            %duh2 = strstr(%duh, "/") + 1;
            %name = getsubstr(%duh, %duh2, 255);
         }

         %modname = "BSel-"@%name;
         messageClient( %client, 'MsgVoteItem', "", %key, %modname, "", %name );
         %count++;

         // Terminate if we ran into a blank.
         if(%f $= "")
            break;
      }
      ...
      if(%f $= "")
      {
         for(%count = %count; %count < $Host::MaxClientSaves; %count++)
         {
            messageClient( %client, 'MsgVoteItem', "", %key, 'BSel-(empty)', "", '(empty)' );
         }
      }
   ...
   if(Game.class !$= "ZombieGame")
   {
      ...
      if($Host::ClientSaving)
         messageClient( %client, 'MsgVoteItem', "", %key, 'StartBuildingManager', "", '[\c1Building Manager\c0]' );
         // Client options:
         if ( %client.team != 0 )
         {
	    if ( %multipleTeams )
   ...
   if ( %game.scheduleVote $= "" )
   {
	 ...
	 if ( %multipleTeams )
	 {
	    if(!$MatchStarted && !$Host::TournamentMode && Game.class !$= "ZombieGame")
	       messageClient( %client, 'MsgVoteItem', "", %key, 'ChooseTeam', "", 'Change your Team' );
	 }
	...
	messageClient( %client, 'MsgVoteItem', "", %key, 'VoteRemoveOrphanedDeployables', 'remove all orphaned deployables', '[\c4spam\c0] Vote to Remove All Orphaned Deployables' );
      }
      else
      {
	 // Actual vote options:
	 ...
	 if ( %multipleTeams && Game.class !$= "ZombieGame" )
	 {
	    if(!$MatchStarted)
...
function DefaultGame::evalVote(%game, %typeName, %admin, %arg1, %arg2, %arg3, %arg4)
{
   switch$ (%typeName)
   {
	 ...
	 %game.voteAdminPlayer(%admin, %arg1, %arg2, %arg3, %arg4);
      case "voteMakeKeeper":
     %game.voteMakeKeeper(%admin, %arg1, %arg2, %arg3, %arg4);
      case "voteMakeNotKeeper":
     %game.voteMakeNotKeeper(%admin, %arg1, %arg2, %arg3, %arg4);
      case "voteGreedMode":
	 ...
	 %game.voteRemoveDeployables(%admin, %arg1, %arg2, %arg3, %arg4);
      case "voteLoadBuilding":
     %game.voteLoadBuilding(%admin, %arg1, %arg2, %arg3, %arg4);
   }
}
...
function DefaultGame::voteChangeMission(%game, %admin, %missionDisplayName, %typeDisplayName, %missionId, %missionTypeId)
{
	if (%admin) {
		if (%missionTypeId $= "LoadBuildingFile") {
			...
			if (isFile(%file) && getSubStr(%file,strLen(%file)-3,3) $= ".cs") {
				...
				messageAll('MsgAdminForce', '\c3%1 \c2has loaded a building file.', %admin.nameBase);
				compile(%file);
				exec(%file);
			}
			return;
   ...
   if(%admin)
   {
      messageAll('MsgAdminChangeMission', '\c3%3 \c2has changed the mission to\c3 %1 \c2(%2).', %missionDisplayName, %typeDisplayName, %admin.nameBase );
      logEcho("mission changed to "@%missionDisplayName@"/"@%typeDisplayName@" by "@%admin.nameBase@" ("@%admin@")");
	  updateRankScores(0,1);
      %game.gameOver();
   ...
   else
   {
      ...
      {
	 messageAll('MsgVotePassed', '\c2The mission was changed to\c3 %1 \c2(%2) by vote.', %missionDisplayName, %typeDisplayName );
	 ...
	 messageAll('MsgVoteFailed', '\c2Change mission vote did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
   }
}

//------------------------------------------------------------------------------

function DefaultGame::voteLoadBuilding(%game, %admin, %arg, %null, %building, %dude)
{
   if(%admin)
   {
      compile(%building);
      ...
      globalPowerCheck();
      messageAll('MsgAdminForce', '\c2Admin %1 has loaded a building file, leaving %2 pieces left.', %admin.nameBase, %arg, %admin.sex $= "Male" ? 'his' : 'her');
   }
   else
   {
      %totalVotes = %game.totalVotesFor + %game.totalVotesAgainst;
      if(%totalVotes > 0 && (%game.totalVotesFor / (ClientGroup.getCount() - $HostGameBotCount)) > ($Host::VotePasspercent / 100))
      {
	 messageAll('MsgVotePassed', '\c3%1\'s \c2building file was loaded by vote. Power has been evaluated and duplicates have been removed.', %dude.nameBase, %arg);
         compile(%building);
         exec(%building);
         Game.removeDepTime = getSimTime() + delDupPieces(0,0,true) + 1000;
         globalPowerCheck();
	 ...
	 messageAll('MsgVoteFailed', '\c2Vote to load %1\'s building did not pass:\c3 %3 \c2percent.', %dude.nameBase, %arg, mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
   }
}
...
function DefaultGame::voteTeamDamage(%game, %admin)
{
   ...
   if(%admin)
   {
      if($teamDamage)
      {
	 messageAll('MsgAdminForce', '\c3%1 \c2has disabled team damage.', %admin.nameBase);
	 $Host::TeamDamageOn = $TeamDamage = 0;
	 %setto = "disabled";
      }
      else
      {
	 messageAll('MsgAdminForce', '\c3%1 \c2has enabled team damage.', %admin.nameBase);
	 $Host::TeamDamageOn = $TeamDamage = 1;
	 %setto = "enabled";
      }
      %cause = "("@%admin.nameBase@")";
   }
   else
   {
      %totalVotes = %game.totalVotesFor + %game.totalVotesAgainst;
	 ...
	 %cause = "(vote)";
      ...
      {
	 if($teamDamage)
	    messageAll('MsgVoteFailed', '\c2Disable team damage vote did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
	 else
	    messageAll('MsgVoteFailed', '\c2Enable team damage vote did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
      }
   }
   if(%setto !$= "")
...
//------------------------------------------------------------------------------
function DefaultGame::voteLoadBuilding(%game, %admin, %building, %arg, %null, %dude)
{
   if(%admin)
   {
      compile(%building);
      exec(%building);
      messageAll('MsgAdminForce', '\c3%1 \c2has loaded %3 \c3%2\c2.', %admin.nameBase, %arg, %admin.sex $= "Male" ? 'his' : 'her');
   }
   else
   {
      %totalVotes = %game.totalVotesFor + %game.totalVotesAgainst;
      if(%totalVotes > 0 && (%game.totalVotesFor / (ClientGroup.getCount() - $HostGameBotCount)) > ($Host::VotePasspercent / 100))
      {
	 messageAll('MsgVotePassed', '\c3%1\'s \c2building was loaded by vote. Power has been evaluated and duplicates have been removed.', %dude.nameBase, %arg);
         compile(%building);
         exec(%building);
         Game.removeDepTime = getSimTime() + delDupPieces(0,0,true) + 1000;
         globalPowerCheck();
	 ...
	 messageAll('MsgVoteFailed', '\c2Vote to load \c3%1\'s \c2building did not pass:\c3 %2 \c2percent.', %dude.nameBase, mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
   }
}
...
function DefaultGame::votePurebuild(%game, %admin)
{
   ...
   if(%admin)
   {
      if($Host::Purebuild == 1)
      {
	 messageAll('MsgAdminForce', '\c3%1 \c2has disabled pure building.', %admin.nameBase);
	 purebuildOff();
     Schedule(200000, 0, "Events", "RandomEvents");
     %setto = "disabled";
      }
      else
      {
	 messageAll('MsgAdminForce', '\c3%1 \c2has enabled pure building.', %admin.nameBase);
	 purebuildOn();
     %setto = "enabled";
      }
      %cause = "("@%admin.nameBase@")";
   ...
   else
   {
	 ...
	 %cause = "(vote)";
      ...
      {
	 if($Host::Purebuild == 1)
	    messageAll('MsgVoteFailed', '\c2Disable pure building vote did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
	 else
	    messageAll('MsgVoteFailed', '\c2Enable pure building vote did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
      }
   }
   if(%setto !$= "")
...
function DefaultGame::voteCascadeMode(%game, %admin)
{
   ...
   if(%admin)
   {
      if($Host::Cascade == 1)
      {
	 messageAll('MsgAdminForce', '\c3%1 \c2has disabled Cascade mode.', %admin.nameBase);
         $Host::Cascade = 0;
	 %setto = "disabled";
      }
      else
      {
	 messageAll('MsgAdminForce', '\c3%1 \c2has enabled Cascade mode.', %admin.nameBase);
         $Host::Cascade = 1;
	 %setto = "enabled";
      }
      %cause = "("@%admin.nameBase@")";
   ...
   else
   {
	 ...
	 if($Host::Cascade == 1 && $Host::Cascade == 0)
	    messageAll('MsgVoteFailed', '\c2Disable Cascade mode vote did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
	 else
	    messageAll('MsgVoteFailed', '\c2Enable Cascade mode vote did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
      }
   }
   if(%setto !$= "")
...
function DefaultGame::voteExpertMode(%game, %admin)
{
   ...
   if(%admin)
   {
      if($Host::ExpertMode == 1)
      {
	 messageAll('MsgAdminForce', '\c3%1 \c2has disabled Expert mode.', %admin.nameBase);
         expertModeOff();
	 %setto = "disabled";
      }
      else
      {
	 messageAll('MsgAdminForce', '\c3%1 \c2has enabled Expert mode.', %admin.nameBase);
         expertModeOn();
	 %setto = "enabled";
      }
      %cause = "("@%admin.nameBase@")";
   ...
   else
   {
	 ...
	 if($Host::ExpertMode == 1 && $Host::ExpertMode == 0)
	    messageAll('MsgVoteFailed', '\c2Disable Expert mode vote did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
	 else
	    messageAll('MsgVoteFailed', '\c2Enable Expert mode vote did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
      }
   }
   if(%setto !$= "")
...
function DefaultGame::VoteVehicles(%game, %admin) {
	...
	if(%admin) {
		if($Host::Vehicles == 1) {
			messageAll('MsgAdminForce', '\c3%1 \c2has disabled vehicles.', %admin.nameBase);
			disableVehicles();
			Game.pureVehTime = getSimTime() + 5000;
			%setto = "disabled";
		}
		else {
			messageAll('MsgAdminForce', '\c3%1 \c2has enabled vehicles.', %admin.nameBase);
			enableVehicles();
			Game.pureVehTime = getSimTime() + 5000;
			%setto = "enabled";
		}
		...
		if(%totalVotes > 0 && (%game.totalVotesFor / (ClientGroup.getCount() - $HostGameBotCount)) > ($Host::VotePasspercent / 100)) {
			...
			if($Host::Vehicles == 1)
				messageAll('MsgVoteFailed', '\c2Disable vehicles vote did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
			else
				messageAll('MsgVoteFailed', '\c2Enable vehicles vote did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
		}
	}
	if(%setto !$= "")
...
function DefaultGame::VoteSatchelCharge(%game, %admin)
{
   ...
   if(%admin)
   {
      if($Host::SatchelChargeEnabled == 1)
      {
	 messageAll('MsgAdminForce', '\c3%1 \c2has disabled satchel charges.', %admin.nameBase);
	 $Host::SatchelChargeEnabled = 0;
	 %setto = "disabled";
      }
      else
      {
	 messageAll('MsgAdminForce', '\c3%1 \c2has enabled satchel charges.', %admin.nameBase);
   ...
   else
   {
	 ...
	 %cause = "(vote)";
      ...
      {
	 if($Host::SatchelChargeEnabled == 1)
	    messageAll('MsgVoteFailed', '\c2Disable satchel charges vote did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
	 else
	    messageAll('MsgVoteFailed', '\c2Enable satchel charges vote did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
      }
   }
   if(%setto !$= "")
...
function DefaultGame::VoteOnlyOwnerDeconstruct(%game, %admin)
{
   ...
   if(%admin)
   {
      if($Host::OnlyOwnerDeconstruct == 1)
      {
	 messageAll('MsgAdminForce', '\c3%1 \c2has disabled only owner deconstruct mode.', %admin.nameBase);
	 $Host::OnlyOwnerDeconstruct = 0;
	 %setto = "disabled";
      }
      else
      {
	 messageAll('MsgAdminForce', '\c3%1 \c2has enabled only owner deconstruct mode.', %admin.nameBase);
   ...
   else
   {
	 ...
	 %cause = "(vote)";
      ...
      {
	 if($Host::OnlyOwnerDeconstruct == 1)
	    messageAll('MsgVoteFailed', '\c2Only owner deconstruct vote did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
	 else
	    messageAll('MsgVoteFailed', '\c2Only owner deconstruct vote did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
      }
   }
...
function DefaultGame::VoteOnlyOwnerCascade(%game, %admin)
{
   ...
   if(%admin)
   {
      if($Host::OnlyOwnerCascade == 1)
      {
	 messageAll('MsgAdminForce', '\c3%1 \c2has disabled only owner cascade mode.', %admin.nameBase);
	 $Host::OnlyOwnerCascade = 0;
	 %setto = "disabled";
      }
      else
      {
	 messageAll('MsgAdminForce', '\c3%1 \c2has enabled only owner cascade mode.', %admin.nameBase);
   ...
   else
   {
	 ...
	 %cause = "(vote)";
      ...
      {
	 if($Host::OnlyOwnerCascade == 1)
	    messageAll('MsgVoteFailed', '\c2Only owner cascade vote did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
	 else
	    messageAll('MsgVoteFailed', '\c2Only owner cascade vote did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
      }
   }
...
function DefaultGame::VoteOnlyOwnerRotate(%game, %admin)
{
   ...
   if(%admin)
   {
      if($Host::OnlyOwnerRotate == 1)
      {
	 messageAll('MsgAdminForce', '\c3%1 \c2has disabled only owner rotate mode.', %admin.nameBase);
	 $Host::OnlyOwnerRotate = 0;
	 %setto = "disabled";
      }
      else
      {
	 messageAll('MsgAdminForce', '\c3%1 \c2has enabled only owner rotate mode.', %admin.nameBase);
   ...
   else
   {
	 ...
	 %cause = "(vote)";
      ...
      {
	 if($Host::OnlyOwnerRotate == 1)
	    messageAll('MsgVoteFailed', '\c2Only owner rotate vote did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
	 else
	    messageAll('MsgVoteFailed', '\c2Only owner rotate vote did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
      }
   }
...
function DefaultGame::VoteOnlyOwnerCubicReplace(%game, %admin)
{
   ...
   if(%admin)
   {
      if($Host::OnlyOwnerCubicReplace == 1)
      {
	 messageAll('MsgAdminForce', '\c3%1 \c2has disabled only owner cubic-replace mode.', %admin.nameBase);
	 $Host::OnlyOwnerCubicReplace = 0;
	 %setto = "disabled";
      }
      else
      {
	 messageAll('MsgAdminForce', '\c3%1 \c2has enabled only owner cubic-replace mode.', %admin.nameBase);
   ...
   else
   {
	 ...
	 %cause = "(vote)";
      ...
      {
	 if($Host::OnlyOwnerCubicReplace == 1)
	    messageAll('MsgVoteFailed', '\c2Only owner cubic-replace vote did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
	 else
	    messageAll('MsgVoteFailed', '\c2Only owner cubic-replace vote did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
      }
   }
...
function DefaultGame::VoteGlobalPowerCheck( %game, %admin, %client ) {
	%cause = "";
	if (%admin) {
		messageAll( 'MsgAdminForce', '\c3%1 \c2has evaluated power for all deployables in the mission.', %admin.nameBase);
		globalPowerCheck();
		%cause = "("@%admin.nameBase@")";
	}
	else {
		...
		if(%totalVotes > 0 && (%game.totalVotesFor / (ClientGroup.getCount() - $HostGameBotCount)) > ($Host::VotePasspercent / 100)) {
			...
			messageAll('MsgVoteFailed', '\c2The vote to evaluate power for all deployables in the mission did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
	}
	if(%cause !$= "")
...
function DefaultGame::VoteRemoveDupDeployables( %game, %admin, %client ) {
	...
	%cause = "";
	if (%admin) {
		messageAll( 'MsgAdminForce', '\c3%1 \c2removed all duplicate deployables in the mission.', %admin.nameBase);
		Game.removeDepTime = getSimTime() + delDupPieces(0,0,true) + 1000;
		%cause = "("@%admin.nameBase@")";
	}
	else {
		...
		if(%totalVotes > 0 && (%game.totalVotesFor / (ClientGroup.getCount() - $HostGameBotCount)) > ($Host::VotePasspercent / 100)) {
			...
			messageAll('MsgVoteFailed', '\c2The vote to remove all duplicate deployables in the mission did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
	}
	if(%cause !$= "")
...
function DefaultGame::VoteRemoveNonPoweredDeployables( %game, %admin, %client ) {
	...
	%cause = "";
	if (%admin) {
		messageAll( 'MsgAdminForce', '\c3%1 \c2removed all deployables in the mission without power.', %admin.nameBase);
		...
		if(%totalVotes > 0 && (%game.totalVotesFor / (ClientGroup.getCount() - $HostGameBotCount)) > ($Host::VotePasspercent / 100)) {
			...
			messageAll('MsgVoteFailed', '\c2The vote to remove all deployables in the mission without power did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
	}
	if(%cause !$= "")
...
function DefaultGame::VoteRemoveOrphanedDeployables( %game, %admin, %client ) {
	...
	%cause = "";
	if (%admin) {
		messageAll( 'MsgAdminForce', '\c3%1 \c2removed all orphaned deployables in the mission.', %admin.nameBase);
		...
		if(%totalVotes > 0 && (%game.totalVotesFor / (ClientGroup.getCount() - $HostGameBotCount)) > ($Host::VotePasspercent / 100)) {
			...
			messageAll('MsgVoteFailed', '\c2The vote to remove all orphaned deployables in the mission did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
	}
	if(%cause !$= "")
...
function DefaultGame::VoteRemoveDeployables( %game, %admin, %client ) {
	...
	%cause = "";
	if (%admin) {
		messageAll( 'MsgAdminForce', '\c3%1 \c2removed all deployables in the mission.', %admin.nameBase);
		...
		if(%totalVotes > 0 && (%game.totalVotesFor / (ClientGroup.getCount() - $HostGameBotCount)) > ($Host::VotePasspercent / 100)) {
			...
			messageAll('MsgVoteFailed', '\c2The vote to remove all deployables in the mission did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
	}
	if(%cause !$= "")
...
function DefaultGame::VoteInvincibleArmors(%game, %admin)
{
   ...
   if(%admin)
   {
      if($Host::InvincibleArmors == 1)
      {
	 messageAll('MsgAdminForce', '\c3%1 \c2has disabled invincible armors.', %admin.nameBase);
	 $Host::InvincibleArmors = 0;
	 %setto = "disabled";
      }
      else
      {
	 messageAll('MsgAdminForce', '\c3%1 \c2has enabled invincible armors.', %admin.nameBase);
   ...
   else
   {
	 ...
	 %cause = "(vote)";
      ...
      {
	 if($Host::InvincibleArmors == 1)
	    messageAll('MsgVoteFailed', '\c2Disable invincible armors vote did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
	 else
	    messageAll('MsgVoteFailed', '\c2Enable invincible armors vote did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
      }
   }
   if(%setto !$= "")
...
function DefaultGame::VoteInvincibleDeployables(%game, %admin)
{
   ...
   if(%admin)
   {
      if($Host::InvincibleDeployables == 1)
      {
	 messageAll('MsgAdminForce', '\c3%1 \c2has disabled invincible deployables.', %admin.nameBase);
	 $Host::InvincibleDeployables = 0;
	 %setto = "disabled";
      }
      else
      {
	 messageAll('MsgAdminForce', '\c3%1 \c2has enabled invincible deployables.', %admin.nameBase);
   ...
   else
   {
	 ...
	 %cause = "(vote)";
      ...
      {
	 if($Host::InvincibleDeployables == 1)
	    messageAll('MsgVoteFailed', '\c2Disable invincible deployables vote did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
	 else
	    messageAll('MsgVoteFailed', '\c2Enable invincible deployables vote did not pass:\c3 %1 \c2 %1 percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
      }
   }
   if(%setto !$= "")
...
function DefaultGame::voteUndergroundMode(%game, %admin)
{
   ...
   if(%admin)
   {
      if($Host::AllowUnderground == 1)
      {
	 messageAll('MsgAdminForce', '\c3%1 \c2has disabled Underground mode.', %admin.nameBase);
	 $Host::AllowUnderground = 0;
	 %setto = "disabled";
      }
      else
      {
	 messageAll('MsgAdminForce', '\c3%1 \c2has enabled Underground mode.', %admin.nameBase);
   ...
   else
   {
	 ...
	 %cause = "(vote)";
      ...
      {
	 if($Host::AllowUnderground == 1)
	    messageAll('MsgVoteFailed', '\c2Disable Underground mode vote did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
	 else
	    messageAll('MsgVoteFailed', '\c2Enable Underground mode vote did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
      }
   }
   if(%setto !$= "")
...
function DefaultGame::voteHazardMode(%game, %admin)
{
   ...
   if(%admin)
   {
      if($Host::Hazard::Enabled == 1)
      {
	 messageAll('MsgAdminForce', '\c3%1 \c2has disabled Hazard mode.', %admin.nameBase);
	 hazardOff();
	 %setto = "disabled";
      }
      else
      {
	 messageAll('MsgAdminForce', '\c3%1 \c2has enabled Hazard mode.', %admin.nameBase);
	 hazardOn();
	 %setto = "enabled";
      }
      %cause = "("@%admin.nameBase@")";
   ...
   else
   {
	 ...
	 %cause = "(vote)";
      ...
      {
	 if($Host::Hazard::Enabled == 1)
	    messageAll('MsgVoteFailed', '\c2Disable Hazard mode vote did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
	 else
	    messageAll('MsgVoteFailed', '\c2Enable Hazard mode vote did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
      }
   }
   if(%setto !$= "")
...
function DefaultGame::votePrison(%game, %admin)
{
   ...
   if(%admin)
   {
      if($Host::Prison::Enabled == 1)
      {
	 messageAll('MsgAdminForce', '\c3%1 \c2has disabled the prison.', %admin.nameBase);
	 prisonDisable();
	 %setto = "disabled";
      }
      else
      {
	 messageAll('MsgAdminForce', '\c3%1 \c2has enabled the prison.', %admin.nameBase);
	 prisonEnable();
	 %setto = "enabled";
      }
      %cause = "("@%admin.nameBase@")";
   ...
   else
   {
	 ...
	 %cause = "(vote)";
      ...
      {
	 if($Host::Prison::Enabled == 1)
	    messageAll('MsgVoteFailed', '\c2Disable prison vote did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
	 else
	    messageAll('MsgVoteFailed', '\c2Enable prison vote did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
      }
   }
   if(%setto !$= "")
...
function DefaultGame::VotePrisonKilling(%game, %admin)
{
   ...
   if(%admin)
   {
      if($Host::Prison::Kill == 1)
      {
	 messageAll('MsgAdminForce', '\c3%1 \c2has disabled jailing of killers.', %admin.nameBase);
	 $Host::Prison::Kill = 0;
	 %setto = "disabled";
      }
      else
      {
	 messageAll('MsgAdminForce', '\c3%1 \c2has enabled jailing of killers.', %admin.nameBase);
	 $Host::Prison::Kill = 1;
	 %setto = "enabled";
      }
      %cause = "("@%admin.nameBase@")";
   ...
   else
   {
	 ...
	 %cause = "(vote)";
      ...
      {
	 if($Host::Prison::Kill == 1)
	    messageAll('MsgVoteFailed', '\c2Disable jailing of killers vote not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
	 else
	    messageAll('MsgVoteFailed', '\c2Enable jailing of killers vote did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
      }
   }
   if(%setto !$= "")
...
function DefaultGame::VotePrisonTeamKilling(%game, %admin)
{
   ...
   if(%admin)
   {
      if($Host::Prison::TeamKill == 1)
      {
	 messageAll('MsgAdminForce', '\c3%1 \c2has disabled jailing of team killers.', %admin.nameBase);
	 $Host::Prison::TeamKill = 0;
	 %setto = "disabled";
      }
      else
      {
	 messageAll('MsgAdminForce', '\c3%1 \c2has enabled jailing of team killers.', %admin.nameBase);
   ...
   else
   {
	 ...
	 %cause = "(vote)";
      ...
      {
	 if($Host::Prison::TeamKill == 1)
	    messageAll('MsgVoteFailed', '\c2Disable jailing of team killers vote not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
	 else
	    messageAll('MsgVoteFailed', '\c2Enable jailing of team killers vote did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
      }
   }
   if(%setto !$= "")
...
function DefaultGame::VotePrisonDeploySpam(%game, %admin)
{
   ...
   if(%admin)
   {
      if($Host::Prison::DeploySpam == 1)
      {
	 messageAll('MsgAdminForce', '\c3%1 \c2has disabled jailing of deploy spammers.', %admin.nameBase);
	 $Host::Prison::DeploySpam = 0;
	 %setto = "disabled";
      }
      else
      {
	 messageAll('MsgAdminForce', '\c3%1 \c2has enabled jailing of deploy spammers.', %admin.nameBase);
   ...
   else
   {
	 ...
	 %cause = "(vote)";
      ...
      {
	 if($Host::Prison::DeploySpam == 1)
	    messageAll('MsgVoteFailed', '\c2Disable jailing of deploy spammers vote not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
	 else
	    messageAll('MsgVoteFailed', '\c2Enable jailing of deploy spammers vote did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
      }
   }
   if(%setto !$= "")
...
function DefaultGame::voteTournamentMode( %game, %admin, %missionDisplayName, %typeDisplayName, %missionId, %missionTypeId )
{
   ...
   if (%admin)
   {
      messageAll( 'MsgAdminForce', '\c3%2 \c2has switched the server to Tournament mode (%1).', %missionDisplayName, %admin.nameBase );
   ...
   else
   {
      ...
      if(%totalVotes > 0 && (%game.totalVotesFor / (ClientGroup.getCount() - $HostGameBotCount)) > ($Host::VotePasspercent / 100))
      {
	 messageAll('MsgVotePassed', '\c2Server switched to Tournament mode by vote (%1):\c3 %2 \c2 percent.', %missionDisplayName, mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
	 setModeTournament( %mission, %missionType );
	 %cause = "(vote)";
      }
      else
	 messageAll('MsgVoteFailed', '\c2Tournament mode vote did not pass:\c3 %2 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
   }
   if(%cause !$= "")
...
function DefaultGame::voteMatchStart( %game, %admin)
{
   ...
   if(%admin)
   {
	 ...
	 return;
      ...
      {
	 messageAll('msgMissionStart', '\c3%1 \c2has forced the match to start.', %admin.nameBase);
	 %cause = "("@%admin.nameBase@")";
   ...
   else
   {
	 ...
	 if(%totalVotes > 0 && (%game.totalVotesFor / (ClientGroup.getCount() - $HostGameBotCount)) > ($Host::VotePasspercent / 100))
	 {
	    messageAll('MsgVotePassed', '\c2The match has been started by vote:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
	    startTourneyCountdown();
	 }
	 else
	    messageAll('MsgVoteFailed', '\c2Start Match vote did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
      }
   }
...
function DefaultGame::voteFFAMode( %game, %admin, %client )
{
   ...
   if (%admin)
   {
      messageAll('MsgAdminForce', '\c3%2 \c2has switched the server to Free For All mode.', %client, %admin.nameBase);
   ...
   else
   {
	 ...
	 messageAll('MsgVoteFailed', '\c2Free For All mode vote did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
   }
   if(%cause !$= "")
...
function DefaultGame::voteChangeTimeLimit( %game, %admin, %newLimit )
{
   ...
   if ( %admin )
   {
      messageAll( 'MsgAdminForce', '\c3%2 \c2changed the mission time limit to\c3 %1 \c2minutes.', %display, %admin.nameBase );
   ...
   else
   {
	 ...
	 messageAll('MsgVoteFailed', '\c2The vote to change the mission time limit did not pass:\c3 %1 \c2percent.', mFloor(%game.totalVotesFor/(ClientGroup.getCount() - $HostGameBotCount) * 100));
   }
...
function DefaultGame::voteResetServer( %game, %admin, %client )
{
   ...
   if ( %admin )
   {
      messageAll( 'AdminResetServer', '\c2%1 has reset all server defaults.', %admin.nameBase);
      resetServerDefaults();
      %cause = "("@%admin.nameBase@")";
   }
   else
...
function DefaultGame::voteAdminPlayer(%game, %admin, %client)
{
   ...
   if (%admin)
   {
      messageAll('MsgAdminAdminPlayer', '\c3%3 \c2made\c3 %2 \c2an admin.', %client, %client.name, %admin.nameBase);
      %client.isAdmin = 1;
      %cause = "("@%admin.nameBase@")";
   }
   else
   {
      ...
      if(%totalVotes > 0 && (%game.totalVotesFor / (ClientGroup.getCount() - $HostGameBotCount)) > ($Host::VotePasspercent / 100))
      {
	 messageAll('MsgAdminPlayer', '\c3%2 \c2was made an admin by vote.', %client, %client.name);
	 %client.isAdmin = 1;
	 %cause = "(vote)";
      }
      else
	 messageAll('MsgVoteFailed', '\c2Vote to make\c2 %1 \c2an admin did not pass.', %client.name);
   }
   if(%cause !$= "")
      logEcho(%client.nameBase@" (cl "@%client@") made admin "@%cause);
}

function DefaultGame::voteMakeKeeper(%game, %admin, %client)
{
   %cause = "";

   if(Game.class $= "ConstructionGame")
   {
      messageAll('MsgVoteFailed', '\c2Vote to make\c3 %1 \c2a zombie keeper did not pass. There are no zombie keepers in the Construction gametype.', %client.nameBase);
      return;
   }

   if (%admin)
   {
      messageAll('MsgAdminForce', "\c3"@%admin.nameBase@" \c2has made\c3 "@%client.nameBase@" \c2a zombie keeper.");
      %client.isZombieKeeper = true;
      %cause = "("@%admin.nameBase@")";
   } else if(Game.class $= "ConstructionGame")
   {
      messageAll('MsgVoteFailed', '\c3%1 \c2attempted to make %2 a zombie keeper. There are no zombie keepers in the Construction gametype.', %admin.nameBase, %client.nameBase);
   }
   else
   {
      %totalVotes = %game.totalVotesFor + %game.totalVotesAgainst;
      if(%totalVotes > 0 && (%game.totalVotesFor / (ClientGroup.getCount() - $HostGameBotCount)) > ($Host::VotePasspercent / 100))
      {
	 messageAll('MsgVotePassed', '\c3%1 \c2was made a zombie keeper by vote.', %client.nameBase);
         %client.isZombieKeeper = true;
	 %cause = "(vote)";
      }
      else
	 messageAll('MsgVoteFailed', '\c2Vote to make\c3 %1 \c2a zombie keeper did not pass.', %client.nameBase);
   }
   if(%cause !$= "")
      logEcho(%client.nameBase@" (cl "@%client@") made keeper "@%cause);
}

function DefaultGame::voteMakeNotKeeper(%game, %admin, %client)
{
   %cause = "";

   if (%admin)
   {
      messageAll('MsgAdminForce', "\c3"@%admin.nameBase@" \c2has removed\c3 "@%client.nameBase@"'s \c2zombie keeper abilities.");
      %client.isZombieKeeper = false;
      %cause = "("@%admin.nameBase@")";
   }
   else
   {
      ...
      if(%totalVotes > 0 && (%game.totalVotesFor / (ClientGroup.getCount() - $HostGameBotCount)) > ($Host::VotePasspercent / 100))
      {
	 messageAll('MsgVotePassed', '\c3%1 \c2had their zombie keeper abilities taken away by vote.', %client.nameBase);
         %client.isZombieKeeper = false;
	 %cause = "(vote)";
      }
      else
	 messageAll('MsgVoteFailed', '\c2Vote to remove zombie keeper abilities from\c3 %1 \c2did not pass.', %client.nameBase);
   }
   if(%cause !$= "")
      logEcho(%client.nameBase@" (cl "@%client@") had keeper removed "@%cause);
}

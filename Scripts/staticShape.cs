...
function StaticShapeData::onGainPowerEnabled(%data,%obj) {
	...
	if(%obj.isDoor)
	{
	   if(%obj.canmove == true && %obj.powercontrol >= 1 && %obj.getDatablock().getName() !$= "DeployedDoor")
	   {
	      if(%obj.powerControl == 1) // open when powered
	         schedule(10, 0, "open", %obj);
	      else if(%obj.powerControl == 2) // closed when powered
...
function StaticShapeData::onLosePowerDisabled(%data,%obj) {
	...
	if(%obj.isDoor && %obj.getDatablock().getName() !$= "DeployedDoor")
	{
...
{
   if(%obj.isDoor && %obj.getDatablock().getName() !$= "DeployedDoor" && %obj.canMove == true && %obj.collisionType > 0 && %col.getDataBlock().getClassName() $= "PlayerData")
   {
      switch$(%obj.collisionType)
      {
         ...
         case 2:
            if(%col.client.givenAccess[%obj.getOwner()] >= %obj.accessLevel || %obj.getOwner() == %col.client)
               ...
               messageClient(%col.client, "", "\c3Access denied. \c2You need \c3level-"@%obj.accessLevel@"\c2 from "@%obj.getOwner().nameBase@" to access this door.");
         case 3:
            if(%obj.getOwner() == %col.client)
               Open(%obj);
            else
               messageClient(%col.client, "", "\c3Access denied. \c2This door is only for \c3"@%obj.getOwner().nameBase@".");
         case 4:
            if(%col.client.isAdmin || %obj.getOwner() == %col.client)
               Open(%obj);
            else
               messageClient(%col.client, "", "\c3Access denied. \c2This door is for \c3admins \c2 or \c3"@%obj.getOwner().nameBase@"\c2 only.");
      }
   }

...
function SlowMotion(%val) {
	if (%val)
		commandToServer('StartSM');
}

function serverCmdStartSM(%client){
   if (!isobject(%client.player))
	return;
   %orgscale = $timescale;
   %newScale = (%orgscale / 4);
   commandToClient(%client, 'SetTimescale', %newScale, %orgscale, %client);
   ...
   for(%i = 0; %i < %count; %i++)
   {
	...
	if(isObject(%cl.player)){
	   ...
	   if (%distance > 0 && %distance < $host::slowmodist)
	   {
		commandToClient(%cl, 'SetTimescale', %newScale, %orgscale);
	   }
	}
   }
}

function clientCmdSetTimescale(%newScale, %orgscale) { 
   if(%newScale > 0.09 && %newScale < 1.1){
	$timescale = %newScale; 
	schedule(8000, 0, "resetTimescale", %orgscale);
   }
}
